...
Postgres enums back the domain's state machines (`estado_cliente`, `tipo_servicio`, `frecuencia_servicio`, `estado_servicio`, `estado_pago`, `metodo_pago` — defined in `migrations/…create-tablas-iniciales.js`). Zod schemas re-declare the same value sets as the single source of truth on the app side (e.g. `ESTADOS_CLIENTE` in `src/features/clients/schema.ts`) — when adding a new enum value, update both the migration and the corresponding Zod schema.
...
- `services.ts` (where present) — pure business-logic helpers with no I/O, e.g. `calcularEstadoMostrado` in `pagos/services.ts` (derives PENDIENTE → VENCIDO from the due date) and `calcularProximoVencimiento` in `services/services.ts` (computes next due date from frequency).
...
- **pagos** (`pagos` feature) — payments against a client/service; stored `estado` is PENDIENTE/PAGADO, but the UI-displayed state additionally derives VENCIDO client-side when a PENDIENTE payment's date has passed (`calcularEstadoMostrado`) — don't assume the DB `estado` column alone reflects what's shown.
- **finanzas / gestion** — aggregate reporting (income by month/client, outstanding debt, payment history) over `pagos`. The page was recently renamed from `/finanzas` to `/gestion` (see uncommitted change moving `src/app/(dashboard)/finanzas` → `src/app/(dashboard)/gestion`); it still reads from `src/features/finanzas/`.
